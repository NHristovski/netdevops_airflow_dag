...
def remote_ansible_remove_service_dag():
    ...
    def validate_params(**context):
        ...
        print('Service check result: ', service_result)

        # Check if service exists
        if service_result and 'response' in service_result:
            service_response = service_result.get('response')
            if service_response:
                print(f"Service '{service_id}' exists in Maat")

                # Extract router information from supportingResource
                supporting_resources = service_response.get('supportingResource', [])
                ...
                raise AirflowException(
                    f"Service '{service_id}' does not exist in Maat."
                )
        else:
            raise AirflowException(
